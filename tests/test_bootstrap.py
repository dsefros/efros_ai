...
def test_bootstrap_wires_kernel_dependencies(monkeypatch):
    ...
    monkeypatch.setattr(run, "build_redmine_service", lambda settings: "redmine-service")
    ...
    assert kernel.get_service("redmine") == "redmine-service"
...
def test_create_app_lifespan_shuts_down_runtime(monkeypatch):
    ...
    monkeypatch.setattr(run, "build_redmine_service", lambda settings: None)
