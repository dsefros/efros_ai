...
def test_bootstrap_wires_kernel_dependencies(monkeypatch):
    ...
    sys.modules["api.server"] = types.SimpleNamespace(create_app=lambda kernel, model_manager: None)
    ...
    def fake_create_app(kernel, model_manager):
        calls["create_app"] = (kernel, model_manager)
        return DummyApp(kernel=kernel, model_manager=model_manager)
    ...
    assert calls["create_app"] == (kernel, kernel.model_manager)
